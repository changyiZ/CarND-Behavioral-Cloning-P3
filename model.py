...
def get_model(ch, row, col):
    # ch, row, col = 3, 160, 320  # camera format
    _model = Sequential()
    _model.add(Lambda(lambda x: x / 127.5 - 1.,
                      input_shape=(row, col, ch),
                      output_shape=(row, col, ch)))
    _model.add(Convolution2D(16, 8, 8, subsample=(4, 4), border_mode="same"))
    _model.add(ELU())
    _model.add(Convolution2D(32, 5, 5, subsample=(2, 2), border_mode="same"))
    _model.add(ELU())
    _model.add(Convolution2D(64, 5, 5, subsample=(2, 2), border_mode="same"))
    _model.add(Flatten())
    _model.add(Dropout(.2))
    _model.add(ELU())
    _model.add(Dense(512))
    _model.add(Dropout(.5))
    _model.add(ELU())
    _model.add(Dense(1))
